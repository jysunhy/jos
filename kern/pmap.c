...
{
	// Fill this function
	struct Page* result = page_free_list, *last=NULL;
	while(result){
	...
	if(last){
		last->pp_link = (result+3)->pp_link;
	//	last->pp_link = result-4;
	}else {
		page_free_list = (page_free_list+3)->pp_link;
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	// LAB 3: Your code here.
    pte_t *pte;
    uint32_t left = (intptr_t)va;
    uint32_t right = ROUNDUP(left + len,PGSIZE);
    int privi;
    while( left < right) {
        if(left >= ULIM)
            goto err;
        pte = pgdir_walk(env->env_pgdir, (void *)left, 0);
        if(!pte)
            goto err;
        privi = PGOFF(*pte);
        if(!((privi & PTE_P) && (privi & PTE_U) ))
            goto err;
        if((perm & PTE_W) && !(privi & PTE_W) )
            goto err;
        left = ROUNDDOWN(left+PGSIZE,PGSIZE);
    }
